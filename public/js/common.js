...
document.addEventListener('DOMContentLoaded', activatePageTransition)
...
var $posts = {
    ...
    smoothScrollToTop: function(timestamp) {
        var state = $posts.scrollTopAnimation
        if (!state) {
            state = {
                startTime: null,
                duration: 2600,
                startPos: window.pageYOffset || document.body.scrollTop || document.documentElement.scrollTop
            }
            $posts.scrollTopAnimation = state
        }

        if (!state.startTime) state.startTime = timestamp || performance.now()

        var progress = Math.min((timestamp - state.startTime) / state.duration, 1)
        var easeOut = 1 - Math.pow(1 - progress, 3)
        var newPos = state.startPos * (1 - easeOut)
        ...
        blocks.forEach(function(code) {
            ...
            button.addEventListener('click', function() {
                ...
                if (navigator.clipboard && navigator.clipboard.writeText) {
                    navigator.clipboard.writeText(text).then(showSuccess).catch(restore)
                } else {
    ...
    mounted: function() {
        ...
        var clickableTopic = document.getElementById('postTopic')
        clickableTopic && clickableTopic.addEventListener('click', function () {
            window.requestAnimationFrame($posts.smoothScrollToTop)
        })

        var bottomButton = document.querySelector('.post-bottom-topic a')
        bottomButton && bottomButton.addEventListener('click', function (e) {
            e.preventDefault()
            window.requestAnimationFrame($posts.smoothScrollToTop)
        })
